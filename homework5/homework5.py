...
class baseball:
    ...
    def G(self,vx,vy):
        vxy=math.sqrt(vx**2+vy**2)
        Gx=-self.B2m(vxy)*math.sqrt((vx-v_wind)**2+vy**2)*(vx-v_wind)
        Gy=-self.B2m(vxy)*math.sqrt((vx-v_wind)**2+vy**2)*vy
        return Gx,Gy
    def fly(self):
        self.X=[self.x0]
        self.Y=[self.y0]
...
#        r=-self.Y[-2]/self.Y[-1]
    ...
    def ylf(self):
        ...
        self.T=[0]
        while not (self.Y[-1]<self.yFinal and self.Vy[-1]<0):
            newVx=self.Vx[-1]+self.G(vx=self.Vx[-1],vy=self.Vy[-1])[0]*self.dt
            newVy=self.Vy[-1]-g*self.dt+self.G(self.Vx[-1],self.Vy[-1])[1]*self.dt
            self.Vx.append(newVx)
            self.Vy.append(newVy)
            meanVx=0.5*(self.Vx[-1]+self.Vx[-2])
            meanVy=0.5*(self.Vy[-1]+self.Vy[-2])
...
width=3
plt.bar(phi,f,width,color='gray',alpha=1)
plt.bar(phi,z,width,color='red',alpha=0.5)
plt.xlabel(r'$\theta^\circ$')
plt.ylabel('the maximum range')
plt.legend(['with spin','without spin',])
